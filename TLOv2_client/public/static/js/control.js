...
var contestants=[{name:"", score:0}, {name:"", score:0}, {name:"", score:0}, {name:"", score:0}];
...
const quest_loader = {
	...
	load_ques2:function(){
		...
		$.get("/apix/read_file",{file:"static/data/ans.txt"}).then((res) => {
	...
	load_ques5: () => {
		$.get("/apix/read_file", {file: "static/data/5_question.txt"}).then((callback) => {
			callback = b64DecodeUnicode(callback);
			callback = JSON.parse(callback);
...
const round_one = {
	...
	load_question: function(){
		$.get("/static/data/1_question.txt").then((callback) => {
			round_one.questions=b64DecodeUnicode(callback);
			round_one.questions=JSON.parse(round_one.questions);
...
const round_two = {
	...
	load_question: function(){
		$.get("/static/data/2_question.txt").then((callback) => {
	...
	correctCNV: function(){
		...
		send_mess("controller","viewer","correctCNV");
		send_mess("controller","contestants","update");
	},
	wrong:function(){
		send_mess("controller","viewer","wrong");
...
const round_three = {
	...
	// load_question: function(){
	// 	$.get("/apix/read_file",{file:"static/data/3_question.txt"}).then((callback) => {
	...
	load_question: function(){
		$.get("/apix/read_file",{file:"static/data/3_question.txt"}).then((res) => {
			res=b64DecodeUnicode(res);
...
const round_four = {
	...
	load_question:function(){
		$.get("/static/data/4_question.txt").then((callback) => {
